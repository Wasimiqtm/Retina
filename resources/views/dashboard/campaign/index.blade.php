   ...
   success:function(data)
   {
    ...
    if (data) {
        ...
        $.each( data['AdSpace'], function( key, value ) {
         ...
         html +=' <div class="item form-check form-image-check p-0"> <input type="radio" value="'+value.id+'" name="adspot" id="adSpot'+key+'" class="btn-check" onclick="AdSelectedData()"> <label class="carousel-btn-check" for="adSpot'+key+'"> <img src="'+img+'" alt="" class="img-fluid"> <i class="fas fa-check"></i></label><ul class="mt-4 fs-20 fw-normal lh-1-2 list-unstyled"><li>Dimensions '+value.dimention+' Metres</li><li>Hight: '+value.hight+'</li><li>Price: N'+value.price+'</li> <li>Lighting: '+value.lightning+'</li> <li>Brand: '+value.brand+'</li> <li>Medium: '+value.medium+'</li><li>Side of Road: '+value.side_road+'</li><li>Orientation: '+value.orientation+'</li> <li>Clutter: '+value.clutter+'</li> <li>Site Run Up: '+value.runup+'m</li><li>Faces: '+value.faces+'</li> <li>Address: '+value.address+'</li> </ul></div>'
          });
          // html +='</div>';
